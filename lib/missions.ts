...
const ready = fs
  ...
  .then(function (obj: MissionDB) {
    ...
    const allItemNames = wikidb.missions
      .map((mission) =>
        mission.tables.map((table) => table.items.map((item) => item.name))
      )
      .reduce(concat)
      .reduce(concat)
    ...
    console.log(
      `Generated mission entries table of size ${JSON.stringify(entries).length}`
    )
    /*
...
function findByStarItem(
  itemStars: number,
  itemName: string
): Array<MissionEntry> {
  return _.filter(
    wikidb.entries,
    (entry) => entry.itemName === itemName && entry.itemStars === +itemStars
  )
}
