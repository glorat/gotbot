...
bot.on('message', msg => {
  ...
  let onReply = function(msg) {
    if (msg) {
      if (msg === 'EMBED') {
        ...
        context.channel.send(msg, {split:true}).then(bot_response => {
  ...
  /* This is largely redundant due to discord.js split feature but this is retained
  to prevent excessive spamming in a channel
   */
