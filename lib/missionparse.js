    ...
    return fs.readFileAsync(file, 'utf8')
      ...
      .then(function ($) {
        ...
        missionlinks.each(function (i, elem) {
          const a = $(this);
          wikidb.missions.push({name: copyString(a.text()), wiki: copyString(a.attr('href')), stars: stars});
        })
      })
      .catch(function (e) {
        throw e;
...
function parseMissionRow(row) {
  ...
  const units = row.find('td').eq(1).find('span').first().contents().filter(function () {
    return this.type === 'text';
  }).text();

  return {name: copyString(item.attr('title')), units:+units, stars:starStrToStars[starStr]};
}
...
function parseMissionTable(table) {
  ...
  while (row && row.find('td').length === 4) {
    ...
    console.log(`    ${item.units} ${item.name} ${item.stars}* ${ret.cost*ret.runs/item.units}  ${ret.runs/item.units}`);
    row = row.next('tr');
  }
