...
export function getSkillsRanked(skills: BaseSkills) {
    ...
    let mskills = Object.keys(skills)
        .filter((skill) => skills[skill as keyof BaseSkills] !== undefined && skills[skill as keyof BaseSkills]!.core > 0)
        .map((skill) => {
            skills[skill as keyof BaseSkills]!.skill = skill;
            return skills![skill as keyof BaseSkills] as Skill;
        });
