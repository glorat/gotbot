...
return fs.readFileAsync(file, 'utf8')
  ...
  .then(function($) {
    ...
    if (newCrew) {
      fs.renameSync('client/morecrew.json', `client/.morecrew.${+Date.now()}.json`, err=>{if (err) {console.log('Failed to rename:'+x)}});
      console.log('Writing new morecrew.json');
      fs.writeFileSync('client/morecrew.json', JSON.stringify(morecrew) );
    }
