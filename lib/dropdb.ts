  ...
  recs.forEach((rec) => {
    let entry = _.find(
      ...
      (e) =>
        e.code === rec.code &&
        e.level === rec.level &&
        e.itemName === rec.itemName &&
        e.itemStars === rec.itemStars
    )
    if (!entry) {
...
async function findByMissionCode(code: string, level: string) {
  const ret = (await drops.asyncFind(
...
async function findByMissionCodeAndUser(
  code: string,
  level: string,
  userid: string
) {
  const ret = (await drops.asyncFind(
    { code: code, level: level, userid: userid },
    {}
  )) as DropEntry[]
  return reduceEntries(ret)
}
