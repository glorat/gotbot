...
function calcAdjustedSkill(doc, fleet) {
  doc.crew.forEach(c => {
    ...
    chars.skills.forEach(sk => {
      if (c[sk]) {
        c.adj[sk] = {};
        c.adj[sk].base = Math.round(c[sk].base * (1 + ( (fleet.starbase[sk] + doc.base[sk]) * 0.01)));
        c.adj[sk].minroll = Math.round(c[sk].minroll * (1 + ( (fleet.starprof[sk] + doc.prof[sk]) * 0.01)));
        c.adj[sk].maxroll = Math.round(c[sk].maxroll * (1 + ( (fleet.starprof[sk] + doc.prof[sk]) * 0.01)));
      }
    });
  });
  return doc;
