...
bot.on('messageCreate', (msg) => {
  ...
  fleetProm.then((fleet: any) => {
    ...
    if (cli.isCliSentence(content)) {
      ...
      msg.mentions.has(bot.user ?? 'nothing at all') &&
...
bot.on(Discord.Events.InteractionCreate, async (msg) => {
  ...
  function emojify(sym: string): string | Discord.Emoji {
    const emojis = msg.inGuild()
      ? (msg.channel?.guild.emojis ?? msg.client.emojis)
      : msg.client.emojis
    const estat = emojis.cache.find((x: any) => x.name === sym.toLowerCase())
    return estat ? estat : sym
  }
        ...
        .then((msgs) => {
          msgs.forEach((msg) => {
            if (msg.author.id === bot.user!.id) {
              console.log(
                'I replied to a deleted message - will try to delete my reply!' +
                  msg
              )
              msg.delete()
            } else {
...
schedule.scheduleJob(crontab, function () {
  ...
  got.stdout.pipe(
    fs.createWriteStream(cfg.dataPath + 'logs/gotcron.log', { flags: 'a' })
  )
