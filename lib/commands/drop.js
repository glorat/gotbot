  ...
  fn: (argv, context) => new Promise((fulfill, reject) => {
    ...
    function viewMission(entrys) {
      ...
      function entryStat(e) {
        ...
        return [0, e.itemName + starStr(e.itemStars), adjCost(e.itemUnits), foo.toFixed(1)];
      }
    ...
    function handleMission(code) {
      ...
      if (args.runs > 0) {
        ...
        if (total !== expectedTotal) {
          lines.push(`${boldify('ERROR:')} Expected ${expectedTotal} items but you've recorded ${total}. Sometimes schematics drop x2`);
        }
