...
export default new Clapp.Command({
  ...
  fn: async (argv: any, context: API.Context) => {
    ...
    try {
      if (bot) {
        ...
        if (!botServer) {
          ...
          throw new Error("Don't know what server I the bot am managed from")
        } else {
          ...
          if (botServer) {
            ...
            const ret =
              `I am in the following ${bot.guilds.cache.size} servers\n` +
              lines.join(`\n`)
            console.log('Done getting list...')
            return ret
          } else {
            return 'No bot server'
          }
        }
      }
    } catch (e) {
