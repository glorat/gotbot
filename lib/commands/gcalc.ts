    ...
    new Promise((fulfill) => {
      try {
        ...
        db.get(userid).then(function (doc: any) {
          ...
          const gchars = _.map(availCrew, Gauntlet.dbCharToChar).map(
            (c: any) => {
              c.crit =
                5 +
                _.filter(featuredTraits, (t) =>
                  _.contains(charTraits[c.name], t)
                ).length *
                  20
              return c
            }
          ...
          const bestNames = Gauntlet.topChars(
            gchars,
            featuredSkill,
            featuredSkillWeight
          )

          const best = _.filter(gchars, (char) =>
          ...
          if (res[0]) {
            msg =
              `Your best 5 crew for ${featuredSkill ? emojify(featuredSkill) : 'general'} and traits ${featuredTraits.join(', ')}\n   ` +
