    ...
    new Promise((fulfill, reject) => {
      ...
      crewdb.get(userid, context).then((doc: CrewDoc) => {
        ...
        if (args.cmd === 'add') {
          ...
          let charsToSearch = chars
            .allCrewEntries()
            .filter((e) => !_.contains(haveNames, e.name))
          // Filter by stars
          if (argv.flags.stars > 0) {
            charsToSearch = charsToSearch.filter(
              (e) => e.stars === argv.flags.stars
            )
          }
          ...
          fulfill(
            `Sorry ${author}. I don't know how to ${args.cmd} to your crew roster`
          )
        }
      })
  ...
  args: [
    {
      ...
      validations: [
        {
          errorMessage:
            'Must be add, remove, vault, unvault, search, list, collect',
          validate: (value: string) => {
            return Boolean(
              value.match(
                /^add|remove|rename|list|vault|unvault|search|collect$/
              )
            )
          },
        },
