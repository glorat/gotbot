...
// Command function
  fn: (argv, context) => new Promise((fulfill, reject) => {
    ...
    function handleItem(stars, name) {
      ...
      entrys = _.sortBy(entrys, e=> (e.cost * e.runs)/e.itemUnits);
...
/*        colWidths: [
      ...
      function entryStat(e) {
        const foo =  (e.cost * e.runs)/e.itemUnits;
        return [e.name, e.code, e.level, foo.toFixed(1)];
      }

      table.push(['Mission','Code', 'Level', 'Chrons']);
      entrys.map(entryStat).map(x=>table.push(x));
      fulfill('```' + table.toString() + '```\n');
