      ...
      if (args.cmd === 'add') {
        chars.matchOne(function (err, name) {
          if (err) {
            ...
            enrichChar(char, function () {
              ...
              const msg = `Hi ${author}. I have added ${chars.statsFor(char, emojify, boldify)}\nYou now have ${doc.crew.length} crew`;
              fulfill(msg);
            });
          }
        ...
        searchCrewByCharTrait(args, entries, function (err, res) {
          if (err) {
            ...
            const sortedRoster = _.sortBy(matchingRoster, x => -(x.stars * 100 + x.level));
            const lines = sortedRoster.map(char => {
              var ret = chars.statsFor(char, emojify, boldify);
              if (char.vaulted) {
                ret = `~~${ret}~~`;
              }
              return ret;
        ...
        searchCrewByCharTrait(args, charsToSearch, function (err, res) {
          if (err) {
            ...
            const lines = ordered
              ...
              .map(char => chars.statsFor(char, emojify, boldify));
  ...
  args: [
    {
      ...
      validations: [
        {
          errorMessage: 'Must be add, remove, vault, search or list',
          validate: value => {
            return Boolean(value.match(/^add|remove|rename|list|vault|search|collect$/));
          }
        }
      ]
    },
