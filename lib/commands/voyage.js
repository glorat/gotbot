    ...
    try {
      ...
      db.users.findOne(qry, function (err, doc) {
        ...
        function replaceConstrainedCrew(obj, before, after) {
          ...
          ret.crew = ret.crew.map(function(item) { return item == before ? after : item; });
          // Replace the skills
          chars.skills.forEach(sk => {
            ret[sk] = ret[sk].map(function(item) { return item == before ? after : item; });
          });
          return ret;
        }
        ...
        var totalSkills = calcTotalSkills(crewList, skillList);
        const hours = voyage.solveTime(totalSkills, 2500);
              ...
              skillList.forEach(sk => {
                if (after[sk]) { // Matching skill to attempt into
                  constrainedCrew[sk].forEach(before => {
                    ...
                    const maybeHours = voyage.solveTime(maybeSkills, 2500);
