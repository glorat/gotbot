...
async function parseCategoryCrew(): Promise<Array<any>> {
  ...
  const subcatFiles = [
    ...
    'Rare',
    'Super_Rare',
    'Super_Rare?pagefrom=' + superRareCutoff,
    'Legendary',
  ]
    ...
    return await fs
      ...
      .then(function ($: cheerio.Root) {
        ...
        crewlinks.each(function (i, elem) {
          ...
          if (
            a.text() !== 'next page' &&
            a.text() !== 'previous page' &&
            !_.some(wikidb.crewentries, (x) => x.name == a.text())
          ) {
            //console.log(cat + '    ' + a.text() + ' ' + stars);
            let entry = {
              name: copyString(a.text()),
              wiki: copyString(a.attr('href')),
              stars: stars,
            }
...
async function parseWikiCrew() {
  let loadPromise = cfg.useSttCrewEntries
    ? parseSttLoadedCrew()
    : parseCategoryCrew()
