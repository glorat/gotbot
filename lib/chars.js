...
function shortName(name) {
  ...
  const shorter = _.first(parts, parts.length-1).map(x => x.substring(0,1));
  shorter.push(_.last(parts));
...
function statsFor(char, emojify, boldify, opts) {
  ...
  if (opts.textOnly) {
    ...
    const nm = char.vaulted ? `(${short})` : short;
    return [nm, char.stars, char.maxstars, char.level].concat(skills.map(sk=>char[sk] ? char[sk].base : ''));
  }
  else {
