    ...
    return fs.readFileAsync(file, 'utf8')
      ...
      .then(function($) {
        ...
        crewlinks.each(function(i,elem) {
          const a = $(this);
          crewsummary.push({name:a.text(), wiki:a.attr('href'), stars:stars});
        })})
...
function parseCharForTraits($) {
  ...
  traitbox.find('a').each(function() {
    traits.push($(this).text());
  });
  return traits.join(', ');
}

function parseCharPage($, stars) {
  ...
  for (var star=0; star<stars; star++) {
    for (var skill=0; skill<skillCount; skill++) {
      ...
      if (!cell) throw ("No column $col in @$fedata");
      var base = cell.contents().filter(function() {return this.type === 'text'; }).text();
      const basere = /\d+/;
      if (!basere.test(base)) {

        console.log(`Missing ${star+1}/${stars} ${skills[skill]} base in ${base}`);
        base = 0;
      }
      const rollre = /\((\d+).(\d+)\)/;
  ...
  return {stars:stars, skill:skilldata, traits:traits};
