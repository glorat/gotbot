...
function parseWikiCrew2(origDb) {
  const origCrew = _.groupBy(origDb.crewentries, x=>x.name);
  ...
  return Promise.all(crewLoadPromises).then(function() {
    ...
    const all = wikidb.crewentries.map(entry => {
      ...
      if (origCrew[entry.name]
          && origCrew[entry.name][0]
          && origCrew[entry.name][0].mtime === mtime
      ) {

        // Not modified, get from before
        return new Promise(function(resolve)  {
          console.log(`Skipping ${entry.name} unchanged at ${mtime}`);
          _.extend(entry, origCrew[entry.name][0]);
          resolve(entry);
        });
      }
      else {
        return fs.readFileAsync(file, 'utf8')
          .then(cheerio.load)
          .then(function(dom) {
            console.log(`Parsing char ${entry.name} at ${mtime} (was ${origCrew[entry.name][0].mtime})`);
            let ret = parseCharPage(dom, entry);
            ret.mtime = mtime;
            return ret;
          })
          .catch(function(e) {
