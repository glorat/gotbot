...
function parseCharPage($, entry) {
  ...
  for (var row=3; row<=13; row++) {
    ...
    for (var star = 0; star < stars; star++) {
      for (var skill = 0; skill < skillCount; skill++) {
        ...
        skilldata.push({stars: star + 1, level:level, skill: skills[skill], base: +base, min: +rolls[1], max: +rolls[2]});
  ...
  entry.traits = traits;
  entry.char = char;
  entry.image = $(`img[alt="${name}"]`).attr('src');
  entry.headImage = $(`img[alt="${name} Head.png"]`).attr('src');
  return entry;
