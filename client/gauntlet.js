...
;(function (myService) {
  if (typeof module !== 'undefined' && module.exports) {
    ...
    angular.module('myApp').factory('Gauntlet', function (Combinatorics) {
  ...
  function fatigueStrengths(chars, strengths) {
    ...
    function currentBests(sts) {
      var curBestChars = _.range(skillCombos.length).map((skIdx) => {
        var bestIdx = _.max(
          _.range(chars.length),
          (charIdx) => sts[skIdx][charIdx]
        )
  ...
  function bestCharForSkills(chars, sks) {
    return _.max(chars, function (char) {
